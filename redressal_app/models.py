...
def filePathHandlers(request, filename):
    old_filename = filename
    timeNow = datetime.now().strftime('%Y%m%d%H:%M:%S')
    filename = "%s-%s" % (timeNow, old_filename)
    return os.path.join('handlers/', filename)
...
class HandlerModel(models.Model):
    ...
    handler_image = models.ImageField(upload_to='handlers/', null=True, blank=True)
...
class ComplaintModel(models.Model):
    compaint_id = models.AutoField(primary_key=True)
    ...
    compalint_resolved = models.CharField(max_length=255)
    complaint_pending = models.CharField(max_length=255)
    complaint_raised = models.CharField(max_length=255)
